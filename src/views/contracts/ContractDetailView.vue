      ...
      <div class="dashboard-grid">
        ...
        <div class="form-card">
          ...
          <div class="form-grid">
            <div class="form-group">
              ...
              <input
                ...
                :disabled="association.hasActiveContract"
              />
            </div>
            <div class="form-group">
          ...
          <div v-if="!association.hasActiveContract">
            ...
            <p style="color: gray; font-size: 0.85rem; margin-bottom: 1rem;">
              A register invitation will be sent to this email
            </p>
            <div class="form-group">
              <input
                type="email"
                ...
                v-model="contractForm.orgAdminEmail"
              />
            </div>
          </div>
        ...
        <div class="form-card">
          ...
          <div class="users-list" style="margin-top: 1rem;">
            ...
            <div
              v-else-if="associatedUsers.length === 0"
              style="color: gray; text-align: center; padding: 2rem;"
            >
...
const handleDelete = async () => {
  if (!confirm('Are you sure you want to TERMINATE this contract? It will be suspended but the record will remain.')) return;
  try {
    ...
    // Reflect locally so the UI switches to "Reactivate" mode
    association.value.status = 'Canceled';
    association.value.isSuspended = true;
  } catch {
    alert('Error deactivating contract.');
  }
};
...
const handleCreateContract = async () => {
  ...
  try {
    const payload = {
      ...contractForm.value,
      associationId: Number(association.value.id),
    };
    ...
    alert('Contract created! Copy the link generated below the button.');
  } catch (err) {
    console.error(err);
...
.header-card {
  ...
  background-color: #161819; padding: 2rem; border-radius: 16px; margin-bottom: 2rem;
}
...
.section-title {
  color: var(--white); margin: 0 0 0.5rem 0; font-size: 1.5rem;
  border-bottom: 1px solid #2a2e30; padding-bottom: 0.5rem;
}
...
.dark-input {
  ...
  background-color: #2a2e30; border: none; color: white;
  outline: none; font-family: var(--font-main); box-sizing: border-box;
}

.magic-link-box {
  margin-top: 1.5rem; padding: 1rem;
  background-color: rgba(1, 160, 112, 0.1);
  border: 1px dashed var(--emerald-green); border-radius: 8px; word-break: break-all;
}
...
.user-item-clickable {
  background: #2a2e30; padding: 1rem; border-radius: 8px; margin-bottom: 0.5rem;
  display: flex; justify-content: space-between; align-items: center;
  cursor: pointer; transition: transform 0.2s, background-color 0.2s;
}
.user-item-clickable:hover {
  background-color: #363b3e; transform: translateX(5px);
...
.user-role-badge {
  font-size: 0.7rem; color: var(--blue-cerulean);
  border: 1px solid var(--blue-cerulean); padding: 2px 6px; border-radius: 4px;
}
</style>